/* =========================
   消費税法 暗記アプリ（最新版 / PWAなし / 白背景テーマ想定）
   - カテゴリ複数対応
   - 重み関数：シンプル方式のみ（weight = 1 / (1 + max(0, score)))
   - スコア範囲：-5 ～ +10
   - A：解答確認はトグル（再タップで隠す）、×は5問後に再出題
   - B：選択→マスク / 繰り返し / 保存
   - C：一覧→編集（再マスク可）、編集モーダルではマスク文字を可視化
...
(() => {
  ...
  // 危険なHTMLを簡易サニタイズ（編集モーダルに挿入する前に通す）
  ...
  repeatBtn.addEventListener('click', () => {
    if (appState.lastPastedHTML) editor.innerHTML = appState.lastPastedHTML;
  });
